  ...
  <div>
    <el-row type="flex" justify="start">
      <div style="margin-right: 5px" v-for="(mixin, name) in mixins" :key="mixin.name">
        <el-button :type="name == currentMixinName ? 'primary':'default'" @click="showMixinData(name)">{{name}}
        </el-button>
      </div>
      <el-popover placement="right" width="500" trigger="click" v-if="moreMixins.length > 0"
    ...
    <el-row style="margin-top:10px">
      <el-col :xl="10" :lg="16">
        <el-card v-if="currentMixinInfo" shadow="never">
          <el-table :data="includedProperties" :show-header="false" empty-text="No properties" highlight-current-row>
            ...
            <el-table-column>
              <template slot-scope="scope">
                <el-tooltip v-if="scope.store.states.currentRow == scope.row" class="item" effect="dark"
                            placement="bottom"
                  ...
                  <el-select v-else-if="scope.row.type == 'array'" v-model="currentMixin[scope.row.name]" multiple
                             no-match-text="No match text" no-data-text="No data" filterable allow-create
                  ...
                  <input v-else class="el-input__inner" placeholder="Please input"
          ...
          <el-popover placement="bottom" width="700" trigger="click" v-model="popoverAddPropVisible"
            ...
            <el-button style="margin-top: 10px" type="success" icon="el-icon-plus" slot="reference">Add Property
            </el-button>
          </el-popover>
