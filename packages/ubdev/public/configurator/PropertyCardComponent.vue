<template>
  <div v-if="currentRow">
    <el-form ref="propsForm" label-position="left" label-width="10rem" :model="currentRow">
      <el-form-item v-for="prop in currentSchema" :key="prop.name" :label="prop.name+': '" :prop="prop.name"
                    :model="currentRow"
        ...
        <objectControl v-else-if="prop.type === 'object'" :row="currentRow" :propName="prop.name"
                       v-on:setPropValue="setPropValue"></objectControl>
        <el-select v-else-if="prop.type === 'array' && prop.name !== 'mapping'" v-model="currentRow[prop.name]"
                   multiple no-match-text="No match text" no-data-text="No data" filterable allow-create
                   default-first-option
                   placeholder="Add Items">
          <el-option v-for="item in currentRow[prop.name]" :key="item" :label="item" :value="item"/>
        </el-select>
        <el-select v-else-if="prop.enum" v-model="currentRow.dataType" placeholder="Select"
        ...
        <mappingObjectControl v-else-if="prop.name === 'mapping'" :row="currentRow" :propName="prop.name"
                              :schema="schema.mapping.items" v-on:setPropValue="setPropValue"></mappingObjectControl>
        <el-input v-else-if="!prop.enum" :type="prop.type === 'number' ? 'number' : 'text'"
