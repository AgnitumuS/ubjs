...
Ext.define('UB.core.UBPanelMixin', {
    ...
    buildDetailList: function () {
        ...
        details.forEach(function(detail){
            dt = detailObj[detail.entityName];
            detailObj[detail.entityName] = dt = dt || {};
            dt[detail.attribute] = true;
        });
        */
        ...
        this.actions.commandLink  =  new Ext.Action({
                ...
                items: [
                    {
                        ...
                        //value: Ext.String.format('{0}//{1}{2}#{3}', location.protocol, location.host, location.pathname, this.createCommandLink())
                    }
                ]
            ...
            listeners: {
                activate: function(item){
                    ...
                    //item.menu.items.getAt(0).selectText();
    ...
    createCommandLink: function () {
        ...
        return prm.join('&');
        /*
        return Ext.String.format('command={0}{1}{2}{3}{4}', this.commandCode,
            this.instanceID ? Ext.String.format('&id={0}', this.instanceID) : '',
            this.onDesktop ? '&ondesktop=true' : '',
            this.parent ? Ext.String.format('&parent={0}', this.parent) : '',
            this.parentID ? Ext.String.format('&parentid={0}', this.parentID) : '');
            */
    },
    ...
    createMenuItemDetails: function (){
        ...
        for (var i = 0, len = entityDetails.length; i < len; ++i) {
            ...
            details.push(new Ext.Action({
                ...
                iconCls: 'iconDetail',
                text: detail.caption ||
                  UB.format('{0} ({1})',entity.caption,
                      entity.attr(detail.attribute).caption ),
