...
Ext.define('UB.core.UBCommand', {
    ...
    statics: {
        ...
        resultDataRow2Object: function(response, rowNum, fieldList) {
            var
                i = -1,
                len, data, responseFieldList, result = {};
        ...
        addMasterDetailRelation: function(whereListTo, masterFields, detailFields, record) {
            var
                result = whereListTo || {}, i, l, item, filterVal, detailField;
           ...
           if (!url || (url.indexOf('#') < 0)){
            ...
            function parseParam(prm){
                ...
                _.forEach(prm, function(item){
                    ...
                    var pair = item.split('='), name, value;
                    if (pair.length !== 2){
                        throwI();
                    }
                    name = pair[0];
            ...
            /*
            if (!target){
                command.target = $App.getViewport().getCenterPanel();
            } */
            return command;
        }
    },
    ...
    constructor: function(config) {
        ...
        switch (me.commandType){
            ...
            default:
                ...
                //MPV impossible here because of circular reference to errorWindow UB.showErrorWindow(errMsg);
    ...
    showList: function() {
        var
            ...
            me = this,
            config,
            showListParam = UB.core.UBUtil.getByPropertyValue(me.commandData.params, 'entity', me.entity);
    ...
    onFormCodeRun: function(formCode) {
        var
            me = this;
...
//        Ext.create('UB.core.UBFormLoader', {
//            code: formCode,
//            callback: this.onShowFormRun,
//            scope: this
//        });
    },

    showReport: function(){
        var me = this, reportParams, report,
            options;
        reportParams = me.commandData;
        options = me.reportOptions || reportParams.reportOptions || {};

        report = Ext.create('UBS.UBReport', _.defaults(options, {code: reportParams.reportCode, type: reportParams.reportType, params: reportParams.reportParams }));
...
//BVV add for preview form------------------------------------------------25-09-2013
        ...
        if(me.tabId){
            ...
            if (!tab && me.instanceID){ //специально для тестировщиков которые открывают из реестра еще раз только чтосохраненный документ
                //tab = Ext.ComponentQuery.query('basepanel[instanceID=' + me.instanceID + ']');
...
//BVV add for preview form------------------------------------------------25-09-2013
    ...
    showCommandResult: function(result, options) {
        var
            me = this,
            parentName = me.detailAttribute ? UB.format(' ({0}: {1})', UB.i18n('filtr'),
                $App.domainInfo.get(me.entity).attr(me.detailAttribute).caption ) : '',
            history = me.__mip_recordhistory ? ' (' + UB.i18n('istorijaIzmenenij') + ')': '',
            title,
            config,
            disableAutoShow = result.disableAutoShow;

        options = options || {};

        title = options.title ||
           (me.description || (me.formParam ? (me.formParam.caption || me.formParam.description) : null) || $App.domainInfo.get(me.entity).caption) +
           parentName + history;
        ...
        if(!me.createOnly) {
            if(!result.target) {
                ...
                if(!options.isGrid /*&& me.instanceID && !me.addByCurrent*/){
                    var
                        entity = me.entity,
                        last = me.instanceID || 'new-'+(new Date()).getTime(),
                        wndId = UB.format('ub-wnd-{0}-{1}', me.entity, last),
                        wnd = Ext.WindowManager.get(wndId);
                ...
                //config.modal = me.isModal || me.isModalDialog;
                // all windows must be modal
                config.modal = true;

                config.resizable = me.isResizable;
                config.maximizable = me.isMaximizable;
                //config.minimizable = me.isMinimizable;
                config.closable = me.isClosable;
                //config.renderTo = UB.view.Viewport.main.getEl();
                //config.renderTo = UB.view.Viewport.centerPanel.getEl();
                //config.constrainTo  = 'ubCenterViewport';

                result.window = Ext.create('UB.view.BaseWindow', config);
                //UB.view.Viewport.main.items.add(result.window);
