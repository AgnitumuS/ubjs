...
Ext.define('UB.ux.designer.VisualDesigner', {
    ...
    initComponent: function(){
        ...
        /*
        var fff = 0;
        me.testPnl = Ext.create('Ext.panel.Panel',{
            width : 100,
            height: 200,
            //flex: 1,
            ...
            items: [{
                ...
                text: 'test',
                handler: function(){
                    Ext.destroy(me.testPnl.resizer, me.testPnl.dd);
                }
            }
            ],
            listeners: {
                boxready: function(sender){
                    ...
                    me.testPnl.resizer = resizer;

                    var dragTarget = new Ext.Component({
                        el: me.testPnl.getEl(), // resizer.getEl(), testPnl
                        rendered: true,
                        container: me.testPnl.container
                    });
                    ...
                    me.testPnl.dd = new Ext.util.ComponentDragger(dragTarget, ddConfig);
                },
                scope: me
            }
        });
    ...
    initEntityTreePanel: function(){
        var me = this;
        me.entityTreePanel = Ext.create('UB.view.CommandBuilder.EntityTreePanel',{
    ...
    highlightElement : function(el) {
        var me = this, elParent;
        //me.resizeLayer.hide();
        if (el === me.builderPanel.getEl()) { return; }
        if (el) {
    ...
    initComponentTree: function(){
        ...
        tree.on('itemclick', function(tree, record, item, index, e, eOpts) {
            var node = record;
            e.preventDefault();
        ...
        tree.on('itemcontextmenu', function(sender, record, item, index, e) { //itemcontextmenu( this, record, item, index, e
            var xnode = record;
            e.preventDefault();
    ...
    initBaseComponents : function(){
        var me = this, tree, root;
        me.baseComponents = tree = Ext.create('Ext.tree.TreePanel',{
            collapsible: false,
            useArrows: true,
            rootVisible: false,
            cls: "no-leaf-icons no-parent-icons",
    ...
    initPropertyGrid: function(){
        ...
        var contextMenu = Ext.create("Ext.menu.Menu",{items:[{
            ...
            handler : function(item,e) {
                ...
                //delete grid.getSource()[item.record.get('name')];
    ...
    setFormConfig : function(configIn, el) {
        ...
        //Ext.AbstractComponent.updateLayout(el, false);
        if (el.doComponentLayout) { //el.getLayoutTarget().update();   doLayout  updateLayout({isRoot : true})   doComponentLayout( )
        ...
        if (config.items) {
            ...
            //    this.addChildComponent(el, config.items[i]);
            //}
