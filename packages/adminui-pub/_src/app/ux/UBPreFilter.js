...
Ext.define('UB.ux.UBPreFilter', {
  ...
  deleteConfig: function () {
    let me = this
    let configName = me.configBox.getRawValue()
    ...
    Ext.Object.each(me.controls, function (attrName, controls) {
      Ext.Object.each(controls, function (controlName, control) {
        if (controlName === 'filterType') {
          let defKey
          let initPrm = me.initParams[attrName]
  ...
  clearAllFilter: function () {
    let me = this
    let predefinedFilters = UB.ux.Multifilter.predefinedFilters
    Ext.Object.each(me.controls, function (attrName, controls) {
      Ext.Object.each(controls, function (controlName, control) {
        if (controlName !== 'filterType') {
  ...
  loadConfig: function () {
    ...
    let setControlValue = function (control, value) {
  ...
  refreshConfig: function () {
    let me = this
    let lastValue = me.configBox.getValue()
  ...
  reCalcFilters: function () {
    ...
    if (me.ftsTextBox && me.ftsTextBox.getValue()) {
      ...
      let func = me.filtersFunc[me.ftsRecordDateAttr]
      if (func) {
        filters = func()
        if (filters.length > 0) {
  ...
  searchHandler: function () {
    let vForm = this.filterWin.down('form')
    if (!vForm.isValid()) {
      return
    }
    let me = this
  ...
  loadLastConfig: function () {
    let me = this
    let lastConfig = window.localStorage.getItem(UB.ux.UBPreFilter.storeKeyPrefix + me.configName)
  ...
  createFtsPanel: function (entity) {
    let me = this
    let items = []
    let ftsRecordDateAttr = entity.mixins.fts.ftsRecordDateAttr

    let filter = me.getPrevFilter(me.getFilterPrefix('ftsText'))
  ...
  createFilters: function () {
    let me = this
    let entity = $App.domainInfo.get(me.entityCode)
    let filter, groupControls, attribute
    let results = []
    let groups = me.options.groups
    let componentsReady = me.options.componentsReady
    let cbParam = {}
    let ftsPanelCfg
    ...
    if (groups && Ext.isArray(groups)) {
      Ext.Array.each(groups, function (group) {
        if (group.fields && Ext.isArray(group.fields)) {
          ...
          results.push({ title: UB.i18n(group.caption),
            padding: '5 0 0 5',
            flex: 1,
            layout: { type: 'vbox', align: 'stretch' },
            autoScroll: true,
  ...
  createFilter: function (attribute, attrName) {
    let me = this
    let items = []
    let entity = me.entityCode
    let title
    let view = {}

    let initParam = me.initParams[attrName] || {}
    let attributeBase = $App.domainInfo.get(entity).getEntityAttribute(attrName, true)
