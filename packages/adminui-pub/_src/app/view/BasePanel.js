...
Ext.define('UB.view.BasePanel', {
    ...
    /**
     * Name of entity BasePanel created for
     * @deprecated Use BasePanel.domainEntity.code instead
    ...
    initComponent: function () {
        ...
        if (me.tabID){  // opened in tab
            ...
            me.dockedItems.splice(0, 0,
                me.headerPanel =
                    Ext.create('Ext.panel.Header',{ //Ext.panel.Header Ext.panel.Panel
                        border: 0,
                        padding: '3 3 3 8',
                        isHeader: true,
                        layout: 'hbox',
        ...
        //me.updateLockInterval = 40000;
        me.callParent(arguments);
        //console.profileEnd('init');
        //me.timePoint('init component base');
        ...
        */
        ...
        me.on('boxready',function(){
            ...
            if (win){
                ...
                //win.mon(me, 'beforeclose', me.onBeforeClose, me);
            }
            //me.timePoint('boxready');
            if (!me.formDataReady){
        ...
        me.on('afterlayout', function(){
            me.timePoint('afterlayout');
        });
        me.on('afterrender', function(){
            me.timePoint('afterrender');
        });
        */
    ...
    updateLockButton: function(){
        ...
        if (!me.lockButton){
            ...
            if (!header){
                win = me.getFormWin();
                if(win){
                    header =win.getHeader();
                }
            }
            if (me.headerPanel){
                header =  me.headerPanel;
            }
            if (header){
    ...
    deleteLock: function(force){
        var
            removeLock = false,
            me = this,
            promise = Q.resolve(me.isEntityLockable && me.entityLocked),
    ...
    setupParentContainer : function () {
        ...
        if(me.isDetail) {
            ...
            me.on('close', function(){
                ...
                if (tabPanel && tabPanel.isMainTabPanel){
                    return;
                }
    ...
    getMasterForm: function (masterLinkFieldControl) {
        var
            me = this;
        return  UB.view.activeForm ? UB.view.activeForm[me.record.get(masterLinkFieldControl.attributeName)] : null;
    },


    getEntityGridPanel: function (gridPanelOrContainer) {
        return (gridPanelOrContainer instanceof UB.view.EntityGridPanel) ? gridPanelOrContainer : gridPanelOrContainer.down('entitygridpanel');
    },
    ...
    makeGridReadonly: function (grid, isReadOnly) {
        var gridPanel,
            action;
        if (!(gridPanel = this.getEntityGridPanel(grid))) {
            return false;
        }
    ...
    makeGridEditable: function (grid, isEditable) {
        ...
        if (!isEditable) {
            gridPanel.suspendEvent('itemdblclick');
        }
        return true;
    },
    ...
    onControlFocused: function (ctrl) {
        ...
        if (!ctrl.enableKeyEvents) {
            ...
            //ctrl.initEvents();  // xmax: duplicate add event listeners on  control.
        }
    },

    canFocus: function (ctrl) {
        var
            result = (ctrl.xtype === 'gridview');
    ...
    //Ctrl+BACKSPACE - очистить поле, если поле не readOnly
    ...
    //Alt+X          закрыть форму, не переспрашивая
    ...
    updateActions: function(command, commandPrm){
        ...
        function syncKeyMap(actionName, disabled){
           switch(actionName){
               case 'aSave':
                   if (kmSave){
                       kmSave.setDisabled(disabled);
                   }
                   break;
               case 'aSaveAndClose':
                   if (kmSaveAndClose){
                       kmSaveAndClose.setDisabled(disabled);
                   }
                   break;
               case 'aDelete':
                   if (kmDelete){
                       kmDelete.setDisabled(disabled);
                   }
                   break;
               case 'aRefresh':
                   if (kmRefresh){
                       kmRefresh.setDisabled(disabled);
                   }
                   break;
           }
        }
    ...
    initEditOrByCurrentFormData: function(){
        ...
        Q.all(requests).done( function(res){
                ...
                //me.timePoint('select executed');
                //$App.connection.convertResponseDataToJsTypes(result);
                if (result && result.resultData.data.length){
                    if (me.addByCurrent) {
                        // запись создается на основе выбранной в гриде
                        me.addCreatingToTitle();
                        $App.connection.addNew({
                            ...
                            //execParams:  me.getFieldValues(me.record, me.fieldList, true)
    ...
    initFormData: function (result) {
        ...
        if (result) {
            ...
            me.enableBinder();
            //me.whereList.ID.values.ID = me.instanceID = me.record.get('ID');
            me.fireEvent('recordloaded', me.record);
        }
        ...
        function doInit(){
            ...
            // moved to formDataReady Ext.callback(me.initUBComponent, me);
        }

        //me.timePoint('Form data loaded');
    ...
    bindHotKeys: function () {
        var
            me = this,
            wnd =  me.getFormWin(), //me.target || UB.core.UBApp.getViewport() window.document.body
    ...
    addInitialFieldValues: function(){
         var me = this;
         if (!me.initialFieldValues || !me.isNewInstance){
             return;
         }
    ...
    addDocumentAttrForComplexUBDocumentAttr: function (componentList, attributeNames, len, i){
        ...
        if (fieldNameParts.length > 1){ //check, that attr complex (has '.' in attributeName)
            var associationFieldName = componentList[i].attributeName.substring(0,
                componentList[i].attributeName.lastIndexOf('.'));
            //before the last '.'.
    ...
    setFields: function (record) {
        var
            ...
            fields = me.fields;
        /*
        me.binder.notReadyControls = 1;
        me.binder.controlDataReady();
        return; // xmax
        */
        if (!record) {
            return;
        }
        //me.timePoint('setFields start');
        ...
        //MPV not need - moved to binder by xmax me.notReadyControls = 1;
        ...
        try
        {
            ...
            //me.timePoint('setFields update caption');

        } finally {
              Ext.resumeLayouts(true);
        }
        //me.timePoint('setFields end');

        //me.controlDataReady(null);
    },
    ...
    getFieldValues: function (record, fieldList, withoutAdtDocument) {
        ...
        for (var i = 0, len = fieldList.length; i < len; ++i) {
            if (withoutAdtDocument && this.domainEntity.attr(fieldList[i]).dataType === ADT_DOC) {
                continue;
            }
    ...
    onFileDragDropConfigure: function(config){
        ...
        if (me.isVisible()) {
            _.forEach(me.documents, function (attrConfig, attrName) {
                var ctrl = me.getField(attrName), // me.getUBCmp(UB.core.UBCommand.getUBCmpUBName(attrName)),
                    docValue;
    ...
    createDocActions: function () {
        ...
        /*
        UB.extensionDownloadMessage().then(function(message){
            if (message){
              var test = 1;
              //todo write disable scanner
            }
        });
        */
    },
    ...
    onShowOriginal: function (action) {
        var ctrl = this.getField(action.attribute); // this.getUBCmp(UB.core.UBCommand.getUBCmpUBName(action.attribute));
    ...
    loadInstance: function(){
        ...
        me.formDataReady = false;
        Q.all(request).done( function(res){
            var result = res[0],
                lockInfo = res[1];
            me.disableBinder();
            UB.ux.data.UBStore.resultDataRow2Record(result, me.record);
    ...
    onDeleteAttachment: function (action) {
        var me = this,
            ctrl = me.getField(action.attribute); // me.getUBCmp(UB.core.UBCommand.getUBCmpUBName(action.attribute));
        //MPV - возможно нужно переместить после вопроса Да/Нет????
        ctrl.fireEvent('change');//shatunov: событие о изменении в данном случае надо отправлять до изменения, а не после
    ...
    /**
     * Close self.
     * Since we do non know, we born inside window or as a child panel
    ...
    closeWindow: function (closeForce) {
        var
            me = this,
            wnd = this.getFormWin(); //me.up('window');
