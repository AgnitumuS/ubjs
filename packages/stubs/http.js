...
function ClientRequest(options) {
    ...
    var _http = this.connection = new THTTPClient(options.host, options.port, options.useHTTPS, options.useCompression,
    ...
    Object.defineProperty(this, 'path', {get: function(){return this.options.path}, set: function(val){this.options.path = val}})
}
...
function IncomingMessage(httpClient) {
    ...
    {
        const h = this._http.responseHeaders.split(CRLF),
            me = this;
        h.forEach(function(header){
            if (header) {
                let hPart = header.split(': ', 2);
                if (hPart.length = 2) { me.headers[hPart[0].toLowerCase()] = hPart[1]; }
            }
        });
    }
}
