...
### Added
 - `ubcli generateNginxCfg` will add `includes` directives into generated config - this allows to extend application reverse proxy 
   ...
   See `Extending autogenerated config` in [Reverse proxy tutorial](https://unitybase.info/api/server-v5/tutorial-reverse_proxy_nginx.html)
...
### Fixed
 - `ubcli generateNginxCfg` - use `$remote_addr` instead of `$realip_remote_addr` to set a remoteIPHeader (X-Real-IP by default).
   ...
   correctly define a real client IP address - see example in `Extending autogenerated config` section of
   [reverse proxy tutorial](https://unitybase.info/api/server-v5/tutorial-reverse_proxy_nginx.html)  
