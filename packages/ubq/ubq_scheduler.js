var me = ubq_scheduler;

var fs = require('fs'),
    {argv, LocalDataStore} = require('@unitybase/base');
...
var
    ...
    FILE_NAME_TEMPLATE = '_schedulers.json',
    attrs = me.entity.attributes;

// calculate default value for entity attributes
var attributes = JSON.parse(me.entity.attributes.asJSON);
var defaultValues = _(attributes).toArray().filter('defaultValue').value()
...
function loadOneFile(model, loadedData) {
    var
        fn = model.path + FILE_NAME_TEMPLATE,
        modelName = model.name,
        content, i, l, existedItem, item;
    if (!fs.existsSync(fn)) { return; }
    var schedulersEnabled = (App.serverConfig.application.schedulers && (App.serverConfig.application.schedulers.enabled === false)) ? false : true;
    var FALSE_CONDITION = 'false //disabled in app config';
    try {
