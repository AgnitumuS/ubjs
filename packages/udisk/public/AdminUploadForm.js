...
Ext.define('UDISK.AdminUploadForm', {
    ...
    initComponent: function() {
        ...
        /*
         me.buttonFile = Ext.create('Ext.Component', {
         margin: '5 0 0 5',
         autoEl: {
         tag: 'input',
         multiple: "multiple",
         ...
         sender.getEl().dom.addEventListener('change', this.onFileSelect.bind(me), false);
         },
         scope: me
         }
         });
        me.listCmp = Ext.create('Ext.Component', {
            margin: '5 0 0 5',
            height: 200,
            width: 300,
            html: '<div><div/>'
    ...
    upLoad: function() {
        var
            me = this, inputDom, el, defer, onCancel, activeEl, timeoutId;
    ...
    doUpload: function(inFile, options){
        var me = this, entityID, record, progressBar, params, promise, lockExist = false, name;
        ...
        return promise
            .then(function(){
                //respIns = UB.LocalDataStore.selectResultToArrayOfObjects(respIns);
                //entityID = respIns[0].ID;
                params =  {
                    entity: me.entityName,
                    attribute: 'fileData',
                    origName: name,
    ...
    upLoadFiles: function(files) {
        var
            me = this, i, file, promises = [], fileNames = [], filesObj = {},
            defer = Q.defer();
        ...
        return UDISK.AdminView.checkAccess(me.parentID, 'write', me.entityName )
            ...
            var renamed = [], fi, replaceAction = 'addNew', advancedOpt = {}, adv,
                nameInfo = JSON.parse(response.resultInfo);
            _.forEach(nameInfo,function(info){
                if (info.newName){
                    ...
                    renamed.push(Ext.String.format(UB.i18n('udiskSizeExistNew'), info.name, me.sizeName(info.size), me.sizeName(fi.size)));
                } else {
                    advancedOpt[info.name] = {};
                }
            });
            ...
            if (renamed.length > 0) {
                Ext.Msg.confirm({
                    ...
                    msg: Ext.String.format(UB.i18n('udiskReplaceFilesConfirm'), renamed),
                    fn: function (btn) {
                        if (btn === 'yes') {
                            replaceAction = 'replace';
                            defer.resolve(doUploadInner());
                        }
                        if (btn === 'no') {
                        ...
                        if (btn === 'cancel') {
                            defer.reject(new UB.UBAbortError());
                        }
                    }
                });
            } else {
