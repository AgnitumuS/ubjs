...
### events
  Во время своей работы, форма генерирует некоторые события. Принцип наименования событий: `{{название сущности}}:{{событие}}`. Например `uba_user:refresh`

    - **refresh** - форма обновлена
    - **loaded** - форма загружена.
