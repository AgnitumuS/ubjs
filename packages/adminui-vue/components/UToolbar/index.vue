...
const { mapInstanceFields } = require('../../utils/formBoilerplate/instance')
...
export default {
  ...
  inject: ['$formServices', 'showFormParams'],

  props: {
    /**
     * if set will check validation before save and refresh
     */
    validation: {
      type: Object,
      default: undefined
  ...
  computed: {
    ...mapGetters(['canSave', 'canRefresh', 'canDelete', 'entitySchema', 'fieldList']),
    ...
    mainPanelButtons () {
      return [{
        ...
        iconCls: 'fa fa-save',
        handler: () => this.save(this.validation),
        ...
        handler: () => this.deleteInstance(this.$formServices),
    ...
    dropdownButtons () {
      const buttons = [{
        ...
        handler: () => this.refresh(this.validation),
    ...
    linkToEntity () {
      const query = [
        'cmdType=showForm',
        `entity=${this.$store.getters.entityName}`,
  ...
  methods: {
    ...
    async saveAndClose () {
      await this.save(this.validation)
    ...
    async formConstructorHandler () {
      ...
      if (!this.showFormParams.formCode) {
        this.$notify({
      ...
      const result = await this.$UB.Repository('ubm_form')
        .attrs(['ID', 'code'])
        .where('code', '=', this.showFormParams.formCode)
        .select()
        ...
        cmdData: {
          params: [
            this.$UB.Repository('uba_auditTrail')
              ...
              .where('[entity]', '=', this.$store.getters.entityName)
    ...
    onKeydownActions (e) {
      ...
      if (e.key === 'Delete' && e.ctrlKey) {
        e.preventDefault()
        if (this.canDelete) {
          this.deleteInstance(this.$formServices)
        }
      }
    }
  }
...
<script>
const { validationInjectMixin } = require('../../utils/formBoilerplate/index')
